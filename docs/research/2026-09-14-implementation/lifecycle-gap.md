> 历史前置记录。2026-09-15 已补登记 bank 清理、投影写入/删除共锁及来源 API，并完成新布局原生验证；最新状态见 [运行记录](README.md#2026-09-15-发布准备进展)。下文保留实现前的约束和发现。2026-09-15 已确认产品没有旧用户数据，旧共享开发布局迁移不列入当前待办；其中兼容保护不代表存在待迁移用户。
