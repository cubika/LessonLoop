...
| methodUses | playbookUseRef、taskRef、任务所属身份、method ObjectRef、返回时间；只关联任务与方法版本 |
...
旧回执可继续读取，usage不参与反馈汇总；旧方法关联中的步骤快照在重新生成WorkCase时移除。宿主与核心、Hindsight扩展需一起更新，旧的步骤观察字段不再接受。EffectCase保留问题引用与人工复核，周期回顾保留范围、周期和通知去重状态。
