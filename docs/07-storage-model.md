...
本开发版本采用产品格式 2，按当前格式初始化。格式 1 数据库会明确拒绝启动；本分支没有迁移旧数据，合并前需确认采用新库或另做迁移。历史开发布局的兼容保护保留为诊断，转换工具不列入当前交付范围；当前格式的数据、控制和备份仍按本页保留与恢复。
...
| 数据 | 权威位置 | 用途 |
|---|---|---|
| Material、WorkCase、Experience、Playbook 元数据 | ProductStore | 工作输入、分层主张、身份、依据、发布状态和用户控制 |
