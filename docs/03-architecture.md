...
| reflect、mental models | 复用结构化输出和原生作业生成候选，原生存储保存当前正文 | 定义方法Schema，核对结构、依据和范围，保存发布状态、内容哈希、依据和用户控制 |
...
| Control Plane、原生 CLI | 原生入口用于基线和诊断；适合的内容、编辑组件可接到产品API后复用 | 补方法准备、当前变更说明、控制回执与产品视图；保留修订标记和用户意图 |
...
模块职责不等于从头开发对应能力。当前复用原生生成、存储和作业机制，产品层补合同差异；独立产品记录不复制原生图、向量或执行队列。正文历史和长期 delta 刷新不在当前方法流程中。具体落点与阶段内工作包见[实现计划](13-implementation-plan.md)。

## 可替换边界

工作代理、模型 provider 和 MemoryEngine 分开适配。产品持有 Source、WorkView 缓存、Experience、Playbook 元数据和用户控制；Hindsight 持有原文、事实、图、内部归纳及当前 Playbook 正文。原生 ID、bank 和 score 不成为公开产品身份或可信度。

更换 Agent 只替换宿主接入；更换引擎需要新适配、来源/索引映射、数据迁移和相同合同验收。当前 Hindsight 适配还依赖下文的共库和固定版本扩展，MemoryEngine 边界不表示可以直接换地址或拆分数据库。首期只实现 Hindsight。模型 provider 复用现有 Copilot 登录，与前台工作会话隔离，避免递归采集学习模型自己的输出。
...
自动学习的生成、审查与人工修订审查共用[原生模型处理函数](../src/core/native-model.ts)，统一提交、找回操作、轮询和读取结果；各自的业务审查、修订核对和发布规则仍由调用方负责。学习作业的冻结输入、候选和审查结果集中在同一 payload，提示按需重建，字段与恢复边界见[07](07-storage-model.md#作业写操作与来源绑定)。

发布前核对当前来源、支持经验、方法结构和用户控制。方法修订与投影确认一致后才对外生效；变化说明引用实际新增材料。工作结果再次进入同一管线，统计标签和周报不作为新增依据。人工编辑先进入独立审查候选；检查失败不改正式正文。受检提交直接写已检查内容，不再次生成。原生正文和 JSON 原位更新，读回确认后解除投递屏障。
...
Agent 通过 getGuidance 获取指导。普通问题查询同时运行方法搜索/准备与经验召回，返回完整方法和相关经验；定向查询只返回指定类型。服务核对权限、修订与依据资格，工作 Agent 检查适用性、执行步骤和选择分支。工具结果按授权采集，供后台复盘使用；使用方法无需额外模型复核或逐步续用。输出预算由[07](07-storage-model.md#预算)统一规定。
...
纠正和停用先写产品控制屏障，再调用原生操作。官方 curation 会在来源重处理时重置，独立控制用于保留用户意图。产品只保留当前修订标记和变化说明，不保存正文历史或提供旧版恢复。
...
当前发行中，产品与 Hindsight 必须使用同一个 PostgreSQL 数据库的独立 schema。固定 0.9.2 扩展直接读取产品控制记录，并与核心共享 advisory lock，核对受检提交、投影写入和清理的当前资格。跨 API 操作仍分别提交，不能假设产品写入与原生写入是同一个事务。

适配器不修改 Hindsight 表结构，但包含有明确用途的内部表操作：按已登记 bank 取消尚未开始的原生作业；擦除 bank 后清理没有外键级联的 llm_requests/audit_log，并核对副本残留。实现集中在[hindsight_product.py](../distribution/hindsight_product.py)和[hindsight_playbooks.py](../distribution/hindsight_playbooks.py)。这些依赖随 Hindsight 固定版本维护，组件升级需重验受检提交、取消、清理及恢复。

方法、投影和引擎状态分别确认；控制库丢失时进入维护状态，不创建空控制库后发布旧引擎内容。

暂停先阻止投递，删除再清点工作案例、经验、方法、临时候选、原生副本、投影和未完成作业。保留剩余独立支持时重新判断，完整清理前不报告完成。导出和迁移需一致视图，恢复不得撤销后来发生的纠正与忘记。

## 运行边界

宿主只声明经过验证的注入、显式准备、结果捕获和异步回执能力。已送入模型的文本不能远程收回，模型内部何时采用不可完全观察。方法使用只保留反馈所需的任务与修订关联，不维护分支执行会话，见[04](04-contracts-and-extensions.md#方法使用与结果回传)。
