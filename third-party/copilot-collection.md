...
每个 Copilot 会话通过已有 startTask 幂等绑定一个 taskRef。回复停止、退出和恢复会话不切分任务；新建 Copilot 会话才建立新关联。已删除任务边界 RPC、new/continue 指令、停止状态和边界补交。taskRef 用于会话来源与反馈关联，不代表一个独立工作主题。

学习来源只从 transcript 提交原始 user、agent、tool 材料；postToolUse 和 finalMessage 不再另行入库，避免两条工具采集路径的去重与时间同步。本地保存字节检查点、工具名称关联和注入回执。失败不推进学习检查点，下个 prompt、agentStop 或 sessionEnd 重读并复用核心幂等。缺失、截断或无法关联的工具记录报告采集缺口。官方 retain cursor 的整篇 replace 会改变产品来源与删除语义，未引入。

效果回顾另有检查点与待重试材料：接收当前用户提示、宿主停止原因和transcript中的原始观察，提交到核心的会话结果判断队列。只开回顾不会创建学习Source，判断也不恢复步骤采用追踪；结果来源、人工纠正和清理规则见[接口合同](../docs/04-contracts-and-extensions.md#本地效果回顾接口)。
