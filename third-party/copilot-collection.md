...
官方 retain cursor 针对单个原生会话文档执行 replace/append；失败后的整篇替换不符合产品材料删除与增量提交语义，因此保留产品采集检查点。任务边界、关闭和结果事件由核心处理，关闭在一次数据库事务内提交。材料、工具观察和效果事件复用核心幂等及容量控制，hook 不再保存三份去重清单。本地 JSON 保存检查点、工具关联、注入回执、事件首次时间及待发送的停止／结束回执；首次时间确保 hook 和 transcript 两条路径使用相同提交内容。中断后先补交边界，未完成的采集记为缺口，后续读取继续补收可用材料。
