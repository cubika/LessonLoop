...
$manifest=@{compatibility=@{productSchema=3;protocol=1;activationCheck=1;hindsight=$components.hindsight;postgresql=$components.postgresql;pgvector=$components.pgvector};version="0.0.1-dev";platform="win32-x64";releaseReady=$false;createdAt=[DateTime]::UtcNow.ToString("o");files=@($files)}
