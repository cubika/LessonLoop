...
if ($Action -in @("uninstall", "data")) {
  ...
  $output = & (Join-Path $runtime "python/python.exe") -B (Join-Path $runtime "distribution/runtime.py") $Action --runtime-root $runtime --data-root $active.dataRoot @Arguments
...
if ($Action -eq "cli") {
  & (Join-Path $runtime "python/python.exe") -B (Join-Path $runtime "distribution/runtime.py") $Action --runtime-root $runtime --data-root $active.dataRoot -- @Arguments
} else {
  & (Join-Path $runtime "python/python.exe") -B (Join-Path $runtime "distribution/runtime.py") $Action --runtime-root $runtime --data-root $active.dataRoot @Arguments
}
exit $LASTEXITCODE
