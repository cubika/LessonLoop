...
$active=Get-Content -LiteralPath (Join-Path $program "active.json") -Raw|ConvertFrom-Json
...
$installation=Get-Content -LiteralPath (Join-Path $active.dataRoot "installation.json") -Raw|ConvertFrom-Json
...
if($installation.runtimeRoot -ne $runtime){
  $journal=Get-Content -LiteralPath (Join-Path $active.dataRoot "update-state.json") -Raw|ConvertFrom-Json
...
& (Join-Path $runtime "python/python.exe") (Join-Path $runtime "distribution/runtime.py") $Action --runtime-root $runtime --data-root $active.dataRoot @Arguments
exit $LASTEXITCODE
