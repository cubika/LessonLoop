...
const prompt =
  "Read fixture.txt once. Diagnose the generated client field that disappears after regeneration using the complete playbook supplied by the LessonLoop hook. Choose the relevant branch from the file contents and explain the remaining check. Do not request step unlocking or report completion. If playbook references are present, call lessonloop-preparePlaybook once with taskRef, playbookId and revision to check that explicit retrieval also returns complete guidance. Stop after these tools. If a tool returns an error, report the limitation. Do not guess identifiers, inspect other files, retry tools, edit files, or claim task success.";
