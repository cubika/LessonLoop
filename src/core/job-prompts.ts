...
export function jobQuery(
  ...
  const legacy =
    stage === "compose" ? payload.modelQuery : payload.assessmentQuery;
  if (legacy !== undefined) return legacy;
  if (payload.promptVersion !== 1 || !payload.inputSources)
    throw new JobPromptError("job_prompt_version_unsupported");
  const query =
    stage === "compose"
      ? composeV1(payload, payload.inputSources, job.synthesisTopicId)
      : assessV1(payload, payload.inputSources);
  const expected =
...
function assessV1(j: JobPayload, inputSources: Source[]) {
  return (
