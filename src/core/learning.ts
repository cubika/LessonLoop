...
import { methodBodySchema, workCaseSchema, byteSize, type Material } from "../domain/schema.js";

const sourceEvidence=z.object({sourceIndex:z.number().int().nonnegative().max(191),excerpt:z.string().min(1).max(512),relation:z.enum(["supports","contradicts"])}).strict();
export const experienceDraftSchema = experienceSchema.innerType().omit({ id: true, revision: true, scopeId: true, createdAt: true, updatedAt: true, derivedFrom: true, sourceFingerprints: true, evidence:true }).extend({ evidence:z.array(sourceEvidence).max(3),parentIndexes: z.array(z.number().int().nonnegative()).max(8) }).strict();
const methodDraftSchema = methodBodySchema.omit({ supportRefs: true, change: true }).extend({ experienceIndexes: z.array(z.number().int().nonnegative()).min(1).max(16), replaces: z.object({id:z.string(),revision:z.number().int().positive()}).strict().optional(), changeKind:z.enum(["create","refine","branch","split"]), changeSummary:z.string().min(1).max(1024) }).strict();
export const caseDraftSchema = workCaseSchema.innerType().omit({ id:true,revision:true,scopeId:true,createdAt:true,updatedAt:true,taskRef:true,sourceFamily:true,methodUses:true,evidence:true }).extend({evidence:z.array(sourceEvidence).max(16)});
export const learningOutputSchema = z.object({ workCase:caseDraftSchema.nullable(), experiences:z.array(experienceDraftSchema).max(8), method:methodDraftSchema.nullable(), decisions:z.array(z.object({ disposition:z.enum(["reject","merge","retain_active","retain_held"]), reason:z.string().min(1).max(512) }).strict()).max(8) }).strict().refine(v=>byteSize(v)<=65536,"Learning output exceeds 64 KiB");
export const assessmentSchema = z.object({ acceptedExperienceIndexes:z.array(z.number().int().nonnegative()).max(8), methodSupported:z.boolean(), reasons:z.array(z.string().min(1).max(512)).max(8) }).strict();
export const outputJsonSchema = zodToJsonSchema(learningOutputSchema, { $refStrategy:"none" }) as Record<string,unknown>;
// Constrain model-authored graph references before they reach product validation.
const methodNode=(outputJsonSchema as {properties:{method:{anyOf:Array<{properties?:Record<string,unknown>}>}}}).properties.method.anyOf.find(v=>v.properties)!;
const stepNode=(methodNode.properties!.steps as {items:{properties:Record<string,unknown>}}).items;
const stepIds=Array.from({length:12},(_,i)=>`s${i+1}`);
stepNode.properties.stepId={type:"string",enum:stepIds};
((stepNode.properties.choices as {items:{properties:Record<string,unknown>}}).items.properties).next={type:"string",enum:[...stepIds,"stop"]};
export const assessmentJsonSchema = zodToJsonSchema(assessmentSchema, { $refStrategy:"none" }) as Record<string,unknown>;
export function learningQuery(materials: Material[], existing: unknown[]) {
  let index=0;const sources=materials.map(m=>({sourceFamily:m.sourceIdentity,context:m.context,segments:m.segments.map(s=>({...s,sourceIndex:index++}))}));
